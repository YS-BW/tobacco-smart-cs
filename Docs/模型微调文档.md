...
| 训练框架 | transformers + peft（直接训练，未使用 LLaMA-Factory） |
| 精度 | bf16 |
| 训练耗时 | 约 5 分钟（310 秒） |
...
| 格式 | 文件 | 大小 | 用途 |
|------|------|------|------|
| GGUF Q4_K_M | ollama `tobacco-csa` | 4.7GB | ollama API 推理 |
| MLX 4-bit | `Qwen2.5-7B-tobacco-mlx` | 4.0GB | Apple Silicon 原生推理 |
